...
@media (max-width: 600px) {
    ...
    <section class="camp-section" id="camp">
        ...
        {{-- ===== SECTION GALERI ERFAN ===== --}}
        @if ($galeriErfan->isNotEmpty())
        <section id="galeri-erfan" class="gallery" style="padding-top: 0;">
            <div class="container" data-aos="fade-up">
                <h2 class="section-title">GALERI ERFAN</h2>
                <p class="section-subtitle text-center mb-4">
                    Dokumentasi kegiatan dan momen-momen seru bersama Erfan.
                </p>

                <div class="gallery-slider-wrapper">
                    <button class="gallery-nav left" onclick="slideGalleryGridErfan(-1)">
                        <i class="fas fa-chevron-left"></i>
                    </button>

                    <div class="gallery-scroll-outer">
                        <div class="gallery-scroll-inner" id="gallerySliderErfan">
                            @php $indexE = 0; @endphp
                            @foreach ($galeriErfan as $ge)
                                @if ($ge->images->isNotEmpty())
                                    <div class="gallery-frame text-center" data-index="{{ $indexE }}"
                                        data-aos="fade-up" data-aos-delay="{{ 100 * ($indexE + 1) }}">
                                        <img src="{{ asset('storage/' . $ge->images->first()->image_path) }}"
                                            alt="{{ $ge->title }}" class="gallery-thumbnail"
                                            onclick="openGalleryModalErfan({{ $ge->id }})">
                                        <div class="gallery-caption">
                                            <h5>{{ $ge->title }}</h5>
                                            <p>{{ Str::limit($ge->description ?? 'Galeri kegiatan Erfan', 50) }}</p>
                                        </div>
                                    </div>

                                    <div id="modal-erfan-{{ $ge->id }}" class="gallery-modal">
                                        <div class="modal-content">
                                            <span class="close-btn"
                                                onclick="closeGalleryModalErfan({{ $ge->id }})">&times;</span>
                                            <h3>{{ $ge->title }}</h3>
                                            <div class="modal-slider-wrapper">
                                                <button class="nav-btn left"
                                                    onclick="slideGalleryErfan({{ $ge->id }}, -1)">&#8592;</button>
                                                <div class="modal-slider" id="slider-erfan-{{ $ge->id }}">
                                                    @foreach ($ge->images as $image)
                    ...
                    <button class="gallery-nav right" onclick="slideGalleryGridErfan(1)">
                        <i class="fas fa-chevron-right"></i>
                    </button>
                </div>
            </div>
        ...
        <script>
            function openGalleryModalErfan(id) {
                document.getElementById('modal-erfan-' + id).classList.add('active');
                document.body.style.overflow = 'hidden';
            }
            function closeGalleryModalErfan(id) {
                document.getElementById('modal-erfan-' + id).classList.remove('active');
                document.body.style.overflow = 'auto';
            }
            function slideGalleryErfan(id, direction) {
                const slider = document.getElementById('slider-erfan-' + id);
                slider.scrollBy({ left: 300 * direction, behavior: 'smooth' });
            }
            function slideGalleryGridErfan(direction) {
                const slider = document.getElementById('gallerySliderErfan');
